import os, cv2, copy, json, numpy as np, threading, transforms3d
...
class jsonProcessor():
    ...
    def start(self):
        ...
        for i in range(1364):
            nums.append(i)
        processing_thread = threading.Thread(target=self.process, args=(0, 1, 2, 3, 4, 5, 6, 7, 1363, 1364), daemon=True)
        processing_thread.start()        
        processing_thread.join()
        # pool = Pool(10)
        # result = pool.map(self.process, range(1364))
        # pool.close()
        # pool.join()
        with open('log.txt', 'a') as log:
            print(self.output, file=log)
            print('---', file=log)
        ...
        #set filenum to next
        #send earlier file for processing
        #if new_data_received then append to outputs
        #set new_data_received to false

    def process(self, filenum):
        print('Processing frame: ' + str(filenum))
        frames_landmarks = []

        filename = self.source_folder + '/' + str(filenum) + '.json'
